...
exports.autoSyncAllDevelopmentRequests = autoSyncAllDevelopmentRequests;
 ...
 */
async function autoSyncAllDevelopmentRequests() {
    try {
        console.log("[Auto-Sync] Début de la synchronisation automatique...");
        // Récupérer toutes les demandes
        const result = await storage_1.storage.getDevelopmentRequests();
        if (!result.success) {
        ...
        for (const request of requestsWithGitHub) {
            try {
                const githubStatus = await (0, github_integration_1.syncGitHubIssueStatus)(request.githubIssueNumber);
                if (githubStatus) {
                    // Mettre à jour le statut local si différent
                    const needsUpdate = request.githubStatus !== githubStatus.status ||
                        (githubStatus.closed && request.status !== "closed");
                    if (needsUpdate) {
                        const updateResult = await storage_1.storage.updateDevelopmentRequest(request.id, {
