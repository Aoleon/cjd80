// Test avec AuthService + RefreshTokenService
...
async function bootstrap() {
  console.log('[TEST9] AVANT NestFactory.create() - +AuthService +RefreshTokenService');

  const app = await NestFactory.create(TestModule, {
    logger: ['log'],
  });

  console.log('[TEST9] APRÈS NestFactory.create() - SUCCESS');

  await app.listen(3009);
  console.log('[TEST9] Server listening on port 3009');
}

bootstrap().catch((error) => {
  console.error('[TEST9] ❌ Error:', error);
  process.exit(1);
});
